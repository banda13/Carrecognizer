...
def sort_train_vs_text(source, p_train, p_test, limit, categories = None):
    ...
    for category in categories:
        ...
        print("%d train and %d test data determined in category %s in %s" % (train_count, test_count, category, source))
        ...
        for i in range(limit):
            if i <= train_count:
                copy2(root_dir + category + "/" + files_in_category[i], train_dir + category + "/" + source + files_in_category[i])
            else:
                copy2(root_dir + category + "/" + files_in_category[i], test_dir + category + "/" + source + files_in_category[i])
